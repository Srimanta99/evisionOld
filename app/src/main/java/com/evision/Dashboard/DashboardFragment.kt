...
import com.evision.model.LDPopup
...
class DashboardFragment : Fragment(),DasboardContract.View {
  ...
  //  lateinit var rec_homebrands:RecyclerView
    override fun preparepage(Hotlist: Dasboard) {
      ...
      if (SplashActivity.is_alert_for_delivery)
          callApiforAlerShow()
    }
    private fun callApiforAlerShow() {
        System.out.println("pop1")
        onHTTP().GETCALL(URL.POPALERT, object : OnHttpResponse {
            override fun onSuccess(response: String) {
                val ldpopup = Gson().fromJson(response, LDPopup::class.java)
                if (ldpopup.is_enable.equals("1"))
                // System.out.println("pop2"+response)
                    showAlertforDelivery(ldpopup.text)
            }
    ...
    private fun showAlertforDelivery(text: String) {
        val builder = AlertDialog.Builder(activity)
        builder.setCancelable(false)
        builder.setMessage(text)
                .setCancelable(false)
                .setPositiveButton("OK") { dialog, id ->
                    SplashActivity.is_alert_for_delivery=false
                    System.out.println("pop dismiss")
                }
