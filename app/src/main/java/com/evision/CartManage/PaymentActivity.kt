...
//                .mobileNumberExplanation("SMS is required on this number")
    ...
    private fun NetwordDetect() {
        ...
        val networkInfo: Array<NetworkInfo> = CM.allNetworkInfo
   ...
   private fun formatnumber(number: Int): String{
    ...
    private fun ORDERPLACE() {
        ...
        if (llcard_view.visibility == View.VISIBLE) {
            ...
            if (til_cardname.text.isNullOrEmpty()) {
                til_cardname.requestFocus()
                return
            }
...
//        }
      ...
      //  params.put("delivery_amount",delivery_cost!!)
        ...
        if (llcard_view.visibility == View.VISIBLE) {
            ...
            params.put("ccname",til_cardname.text.toString())
        }
            ...
            override fun onSuccess(response: String) {
                ...
                if (data.code == 200) {
                    ...
                    if (llcard_view.visibility == View.VISIBLE) {
                        ...
                        logindata!!.cartCount = 0
                        ShareData(this@PaymentActivity).SetUserData(Gson().toJson(logindata).toString())
                        val yappyApiResponse = JSONObject(response).optBoolean("yappy_api_response")
                        var yppyurl=JSONObject(response).optString("yappy_payment_url")
                            if (yappyApiResponse) {
                                startActivity(Intent(this@PaymentActivity, PaymentRedirectUrlActivity::class.java).putExtra("url", yppyurl))
                                finish()
                            } else
                                Toast.makeText(this@PaymentActivity, "Something Wrong in yappy payment", Toast.LENGTH_LONG).show()
